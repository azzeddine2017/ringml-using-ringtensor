        ...
        if hasAttribute(oModel, "aBlocks")
            for i = 1 to len(oModel.aBlocks)
                ...
                if hasAttribute(oBlock, "oAttention")
                    ...
                    nSumSq = 0
                    for k=1 to nHeads 
                        nSumSq += tensor_sum_squares(oAttn.aQueryLayers[k].oGradWeights.pData)
                        nSumSq += tensor_sum_squares(oAttn.aQueryLayers[k].oGradBias.pData)
                    next
                    ...
                    nSumSq = 0
                    for k=1 to nHeads 
                        nSumSq += tensor_sum_squares(oAttn.aKeyLayers[k].oGradWeights.pData)
                        nSumSq += tensor_sum_squares(oAttn.aKeyLayers[k].oGradBias.pData)
                    next
                    ...
                    nSumSq = 0
                    for k=1 to nHeads 
                        nSumSq += tensor_sum_squares(oAttn.aValueLayers[k].oGradWeights.pData)
                        nSumSq += tensor_sum_squares(oAttn.aValueLayers[k].oGradBias.pData)
                    next
